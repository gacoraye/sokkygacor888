...
function avoidWin() {
  for (let r = 0; r < rows; r++) {
    ...
    while (attempts < 50) {
      ...
      let hasCombo = false;
      for (let windowSize = 3; windowSize <= Math.min(5, cols); windowSize++) {
        for (let start = 0; start <= cols - windowSize; start++) {
          let slice = rowSymbols.slice(start, start + windowSize);
          let baseSymbols = slice.filter(s => s !== "🃏" && s !== "⭐");
          if (baseSymbols.length === 0) continue;
          let first = baseSymbols[0];
          if (baseSymbols.every(s => s === first)) {
            hasCombo = true;
            break;
          }
        }
      ...
      if (!hasCombo) {
        for (let c = 0; c < cols; c++) {
          let stop = parseInt(reels[c].dataset.stop) || 0;
          reels[c].children[stop + r].textContent = rowSymbols[c];
        }
        break;
      }
      attempts++;
...
function spinStep(){
  ...
  for (let c = 0; c < cols; c++) {
    let stop = Math.floor(Math.random() * (reels[c].children.length - rows));
    reels[c].dataset.stop = stop;
  }
  avoidWin();
  reels.forEach((reel) => {
    reel.style.transition = `transform ${SPIN_DURATION/1000}s ease-out`;
    reel.style.transform = `translateY(-${reel.dataset.stop * cellH}px)`;
  });
  setTimeout(() => reels.forEach(r => r.parentElement.classList.remove("spinning")), SPIN_DURATION);
  highlightReelsGlow();
